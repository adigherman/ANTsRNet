...
InpaintingDeepFillModel <- R6::R6Class("InpaintingDeepFillModel",
                                       ...
                                       public = list(
                                         ...
                                         buildNetwork = function(trainable = TRUE) {
                                           ...
                                           imageInput <- layer_input(
                                             batch_shape =
                                               c(self$batchSize, self$inputImageSize)
                                           )
                                           maskInput <- layer_input(
                                             batch_shape =
                                           ...
                                           modelStage1 <- keras_model(
                                             inputs = list(imageInput, maskInput),
                                             outputs = output
                                           )
                                         ...
                                         train = function(X_train, numberOfEpochs = 200, maskRegionSize = NA,
                                           ...
                                           if (self$dimensionality == 2) {
                                             for (i in seq_len(maskRegionSize[1]))
                                             {
                                               ...
                                               {
                                                 discountMask <- max(
                                                   gamma^min(i, maskRegionSize[1] - i),
                                           ...
                                           for (i in seq_len(numberOfEpochs))
                                           {
                                             ...
                                             if (self$dimensionality == 2) {
                                               X_maskLocalPatch <- X_mask[
                                                 , beginCorner[1]:endCorner[1],
                                                 beginCorner[2]:endCorner[2],
                                                 ]
                                               X_batchLocalPatch <- X_batch[
                                                 , beginCorner[1]:endCorner[1],
                                                 beginCorner[2]:endCorner[2],
                                                 ]
                                               X_maskedLocalPatch <- X_masked[
                                                 , beginCorner[1]:endCorner[1],
                                                 beginCorner[2]:endCorner[2],
                                                 ]
                                               ...
                                               X_completeLocalPatch <- X_complete[
                                                 , beginCorner[1]:endCorner[1],
                                                 beginCorner[2]:endCorner[2],
                                                 beginCorner[3]:endCorner[3],
                                                 ]
                                             ...
                                             localPenalty <- self$gradientPenaltyLoss(interpolatedLocal,
                                                                                      wgandGpInterpolatedModel$localDiscriminator,
                                                                                      mask = localPatchMask
                                             )
                                             globalPenalty <- self$gradientPenaltyLoss(interpolatedGlobal,
                                                                                       wgandGpInterpolatedModel$globalDiscriminator,
                                                                                       mask = mask
                                             )
...
ContextualAttentionLayer2D <- R6::R6Class("ContextualAttentionLayer2D",
                                          ...
                                          public = list(
                                            ...
                                            call = function(inputs, mask = NULL) {
                                              ...
                                              if (length(inputs) < 2) {
                                                errorMessage <- paste0(
                                                  "inputs should consist of the foreground ",
                                              ...
                                              backgroundPatches <- self$tf$reshape(
                                                backgroundPatches,
                                                c(
                                                  self$tf$shape(backgroundTensor)[1], -1L,
                                                  backgroundKernelSize, backgroundKernelSize,
                                                  self$tf$shape(backgroundTensor)[4]
                                                )
                                              )
                                              backgroundPatches <- self$tf$transpose(
                                                backgroundPatches,
                                                c(0L, 2L, 3L, 4L, 1L)
                                              )
                                              ...
                                              resampledBackgroundPatches <- self$tf$reshape(
                                                resampledBackgroundPatches,
                                                c(
                                                  self$tf$shape(resampledBackgroundTensor)[1], -1L,
                                              ...
                                              numberOfIterations <- min(
                                                c(length(resampledForegroundGroups)),
                                              ...
                                              for (i in seq_len(numberOfIterations))
                                              {
                                                ...
                                                output <- self$tf$nn$conv2d(fg, rgNorm,
                                                                            strides = c(1, 1, 1, 1),
                                                                            padding = "SAME"
                                                )
                                                ...
                                                if (self$fusionKernelSize > 0L) {
                                                  fusionKernel <- self$tf$reshape(
                                                    self$tf$eye(self$fusionKernelSize),
                                                    c(self$fusionKernelSize, self$fusionKernelSize, 1L, 1L)
                                                  )

                                                  output <- tf$reshape(output, c(
                                                    1L,
                                                  ...
                                                  output <- tf$reshape(
                                                    output,
                                                    c(
                                                      1L, newForegroundShape[1] * newForegroundShape[2],
                                                ...
                                                output <- self$tf$nn$conv2d_transpose(output, bg,
                                                                                      ...
                                                                                      strides = c(1, self$dilationRate, self$dilationRate, 1)
                                                ) / 4.0
...
layer_contextual_attention_2d <- function(
  ...
  create_layer(
    ContextualAttentionLayer2D, object,
    list(
...
layer_contextual_attention_3d <- function(
  ...
  name = NULL, trainable = FALSE) {
  create_layer(
    ContextualAttentionLayer3D, object,
    list(
      kernelSize = kernelSize, stride = stride,
  ...
  public = list(
    ...
    call = function(inputs, mask = NULL) {
      ...
      backgroundPatches <- self$tf$extract_volume_patches(backgroundTensor,
                                                          ksizes = c(
                                                            1, backgroundKernelSize, backgroundKernelSize,
      ...
      maskPatches <- maskPatches[1, , , , , ]
      maskData <- self$tf$cast(self$tf$equal(self$tf$reduce_mean(maskPatches,
                                                                 axis = c(0L, 1L, 2L, 3L), keep_dims = TRUE
      ), 0.0), self$tf$float32)
      ...
      for (i in seq_len(numberOfIterations))
      {
        ...
        output <- self$tf$nn$conv2d_transpose(output, bg,
                                              ...
                                              strides = c(
                                                1, self$dilationRate, self$dilationRate,
                                                self$dilationRate, 1
                                              )
        ) / 4.0
        outputGroups[[i]] <- output
